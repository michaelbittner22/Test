...
def load_sample_data():
    return pd.read_csv("data/sample_forecast_shipments.csv", parse_dates=["Date"])
...
with tab1:
    ...
    if flagged_df.empty:
        ...
        styled = flagged_df[display_cols].style.applymap(
            bias_color, subset=["Streak_Avg_Bias_Pct", "Overall_Avg_Bias_Pct"]
        ).format({
